        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <table class="table table-light">
                    ...
                    <tbody>
                    @if(count($tarefasUser) == 0)
                        <tr>
                            <td>Você não possui taferas concluídas</td>
                        </tr>
                    @else
                        @foreach($tarefasUser as $tarefa)
                            <tr>
                                ...
                                <td>
                                    <form action = "{{route('tarefa.destroy', $tarefa->id)}}" method = "POST">
                                        @csrf
                                        <a class = "btn" href="{{route('tarefa.edit', $tarefa->id)}}"><img src="{{ asset('imagens/edit.png') }}" title="Editar"></a>
                                        @method('DELETE')
                                        <button type = "submit" class = "btn btn-link"><img src="{{ asset('imagens/delete.png') }}" title="Editar"></button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
